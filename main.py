...
img2 = get_img_as_base64(os.path.join('images', "main7.jpg"))
...
# """ Add Dropdowns for Columns """
    ...
    # """ Add New Tabs """
    tab0, tab2, tab3, tab4 = st.tabs(['**REVENUE**', "**CUSTOMER TREND**", "**ACCOMMODATION - TRANSPORTATION**", "**DATA FRAME**"])
    ...
    # )
            ...
            st.plotly_chart(fig)
            st.markdown('<span style="font-family: SVN-Gilroy; font-size: 20px; font-weight: bold;">AVERAGE REVENUE AND THE NUMBER OF VISITORS CHOOSING THIS DESTINATION</span>', unsafe_allow_html=True)
            st.write('**The provided scatter plot depicts the mean income and the count of individuals opting for this destination. A total of over 100 customers were observed.**')     
    #  fig = go.Figure(go.Funnel(
    #         y = ["Total Customers", "First Time", "Second Time", "Third Time", "Fourth Time","Sixth Time"],
    #         x = [109, 95, 8, 2, 2, 2],
    ...
    with tab3:
        ...
        with col2:
            ...
            fig.update_layout(title= {'text':'','font':{'family':'SVN-Gilroy','size':20}},
                              ...
                              font=dict(family="SVN-Gilroy",size=14),paper_bgcolor='rgba(255, 255, 255, 0)',plot_bgcolor='rgba(255, 255, 255, 0)',
                              width=450, height=450)
            fig.update_traces(showlegend=False)
            st.plotly_chart(fig)
